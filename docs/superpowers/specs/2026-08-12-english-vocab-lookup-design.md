...
這是投報率最高的一項：內容早就在 repo 裡（`public/data/questions.json`，其中英文科 pp-cs-en / pp-im-en 數百題），使用者本來就會在站上讀這些題目，而在準備考試時遇到的生字，定義上就是最該記起來的字。

做法是在既有的 `src/app/[exam]/questions/[questionId]/page.tsx` 把題目文字接上 Task 10 的 tokenizer，讓每個英文字可點。閱讀模式的側欄元件直接複用。
